...
def _ensure_ordered_unique(courses: List[str]) -> List[str]:
    """Return a list of course codes preserving order without duplicates or blanks."""
    seen: set[str] = set()
    ordered: List[str] = []
...
def _courses_differ(first: List[str], second: List[str]) -> bool:
    """Check whether two course selections differ ignoring order."""
...
def _update_course_selection_dirty() -> None:
    """Refresh the dirty flag comparing pending and confirmed selections."""
    ...
    st.session_state.course_selection_dirty = _courses_differ(pending, confirmed)


def _pending_differs_from_confirmed() -> bool:
    """Return whether pending selections differ from confirmed ones."""
    if "course_selection_dirty" not in st.session_state:
        _update_course_selection_dirty()
    return bool(st.session_state.get("course_selection_dirty", False))


def _set_pending_courses(new_courses: List[str], *, merge_with_existing: bool = True) -> None:
    """Persist pending course selections and update dirty flag."""
    normalized = _ensure_ordered_unique(new_courses)

    if merge_with_existing and "pending_courses_to_offer" in st.session_state:
        current = _ensure_ordered_unique(st.session_state.get("pending_courses_to_offer", []))
        merged: List[str] = [course for course in current if course in normalized]
        merged.extend(course for course in normalized if course not in merged)
        normalized = merged
...
def _set_confirmed_courses(new_courses: List[str]) -> None:
    """Persist confirmed course selections and sync legacy state."""
    normalized = _ensure_ordered_unique(new_courses)
...
def _initialize_course_selection_state() -> None:
    """Ensure course selection state keys exist and are normalized."""
    existing_selection = _ensure_ordered_unique(
        st.session_state.get("selected_courses_to_offer", [])
    )

    confirmed = st.session_state.get("confirmed_courses_to_offer")
    if confirmed is None:
        confirmed = list(existing_selection)
    else:
        confirmed = _ensure_ordered_unique(list(confirmed))

    st.session_state.confirmed_courses_to_offer = confirmed
    st.session_state.selected_courses_to_offer = list(confirmed)

    pending = st.session_state.get("pending_courses_to_offer")
    if pending is None:
        pending = list(confirmed)
    else:
        pending = _ensure_ordered_unique(list(pending))

    st.session_state.pending_courses_to_offer = pending
    _update_course_selection_dirty()
...
def course_planning_view():
    ...
    _initialize_course_selection_state()
...
def _render_quick_actions():
    ...
    if st.button("🔄 Clear All Selections", use_container_width=True):
        _set_pending_courses([], merge_with_existing=False)
        st.rerun()
...
def _render_course_selection_table(analysis_df: pd.DataFrame):
    ...
    if st.session_state.get("quick_select_critical", False):
        ...
        _set_pending_courses(updated_pending, merge_with_existing=False)
    ...
    if new_selected_courses != list(pending_courses):
        ...
        if _courses_differ(new_selected_courses, current_pending):
            ...
            _set_pending_courses(preserved + additions, merge_with_existing=False)
        else:
            _set_pending_courses(current_pending, merge_with_existing=False)
    ...
    with controls:
        ...
        with col_apply:
            if st.button(
                ...
                key="apply_course_selection_button"
            ):
                _set_confirmed_courses(list(pending_courses))
                _set_pending_courses(list(pending_courses), merge_with_existing=False)
                st.rerun()
        with col_discard:
            if st.button(
                "Discard Changes",
                use_container_width=True,
                disabled=not pending_differs,
                key="discard_course_selection_button"
            ):
                _set_pending_courses(list(confirmed_courses), merge_with_existing=False)
                st.rerun()
