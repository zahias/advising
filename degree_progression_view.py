...
def degree_progression_view():
    """
    Degree Progression View - organizes courses by Suggested Semester.
    Two modes:
      - All Students (grouped by semester with status codes)
...
def _render_all_students():
    ...
    def render_course_table(label: str, course_codes: list[str], key_suffix: str):
        ...
        # Group courses by Suggested Semester
        semester_groups = {}
        for course in selected:
            course_info = courses_df.loc[courses_df["Course Code"] == course]
            if not course_info.empty:
                suggested_sem = str(course_info.iloc[0].get("Suggested Semester", "Unspecified")).strip()
                if not suggested_sem or suggested_sem.lower() in ["nan", "none", ""]:
                    suggested_sem = "Unspecified"
            else:
                suggested_sem = "Unspecified"
            
            if suggested_sem not in semester_groups:
                semester_groups[suggested_sem] = []
            semester_groups[suggested_sem].append(course)
        
        # Sort semester groups by year and semester order
        def parse_semester(sem_str):
            if sem_str == "Unspecified":
                return (999, 999)  # Sort to end
            parts = sem_str.split("-")
            if len(parts) != 2:
                return (999, 999)
            semester, year = parts[0], parts[1]
            try:
                year_num = int(year)
            except:
                year_num = 999
            ...
            sem_num = sem_order.get(semester, 999)
            return (year_num, sem_num)
        
        sorted_semesters = sorted(semester_groups.keys(), key=parse_semester)
        
        # Build full table with semester sections
        all_export_dfs = []
        ...
        if all_export_dfs:
            # Get base columns from first table
            export_df = all_export_dfs[0][base_display_cols].copy()
            # Add all course columns from all semester tables
            for df_semester in all_export_dfs:
                course_cols = [col for col in df_semester.columns if col not in base_display_cols]
                for col in course_cols:
                    export_df[col] = df_semester[col]
        else:
            export_df = None
