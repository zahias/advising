...
def _parse_placement_file(content: bytes) -> pd.DataFrame:
    ...
    df = df[df['student_id'] != '' ]
    df = df[df['placement_course'] != '']
    return df[['student_id', 'placement_course']].drop_duplicates(subset=['student_id'])


def _build_intensive_ancestor_map(courses_df: pd.DataFrame) -> dict[str, set[str]]:
    """Return  {course_code: set_of_ancestor_intensive_courses}.

    An ancestor is any intensive course that must be completed *before* this one,
    derived transitively from the Prerequisite column.
    """
    from app.legacy.eligibility_utils import parse_requirements  # noqa: PLC0415
    ...
    # Build direct prerequisite graph restricted to intensive courses
    ...
    # BFS / DFS to get transitive ancestors for each course
    def _ancestors(code: str) -> set[str]:
        visited: set[str] = set()
        stack = list(prereqs.get(code, []))
        while stack:
            current = stack.pop()
            if current in visited:
                continue
            visited.add(current)
            stack.extend(prereqs.get(current, []) - visited)
        return visited

    return {code: _ancestors(code) for code in intensive_codes}
...
def bulk_placement_from_file(
    ...
    ancestor_map = _build_intensive_ancestor_map(courses_df)
    intensive_codes = set(ancestor_map.keys())

    processed = 0
    errors: list[str] = []

    for _, row in df.iterrows():
        student_id: str = str(row['student_id'])
        placement: str = str(row['placement_course'])

        if placement not in intensive_codes:
            errors.append(
                f'Student {student_id}: placement course "{placement}" is not an intensive course — skipped.'
            )
            continue

        courses_to_exclude = sorted(ancestor_map[placement])

        # Load existing non-intensive exclusions so we don't clobber them
        from app.models import CourseExclusion, Major
        from sqlalchemy import select
        major = session.scalar(select(Major).where(Major.code == major_code))
        if not major:
            errors.append(f'Major {major_code} not found.')
            break
        ...
        # Preserve exclusions for non-intensive courses and merge new intensive exclusions
        non_intensive_excl = existing_excl - intensive_codes
        final_excl = sorted(non_intensive_excl | set(courses_to_exclude))
