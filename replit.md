...
## Recent Changes
- **2025-11-03**: Replaced Course Dependency Tree with Degree Plan View. New view shows semester-by-semester curriculum grid (Fall 1, Spring 1, Summer 1, Fall 2, Spring 2, Fall 3, Spring 3) based on official degree plan structure. Color codes student progress: green=completed, yellow=registered, blue=available, gray=not eligible, red=failed. Supports optional "Suggested Semester" column in courses_table.xlsx (explicitly prioritizes this over "Semester Offered" to avoid column confusion); defaults to PBHL official curriculum structure from PDF pages 9-10 when no semester column exists.
