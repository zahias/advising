...
## Recent Changes
- **2025-11-06**: Added Course Offering Planner - a smart recommendation engine that prioritizes courses based on graduating students (remaining credits threshold), bottleneck analysis (downstream prerequisite unlocks), currently eligible students, and cascading eligibility effects. Features Top 10 recommendations with visual priority indicators, detailed impact metrics, and interactive course selection with impact summaries.
- **2025-11-06**: Consolidated Degree Progression functionality into Full Student View by adding semester filtering. Removed separate Degree Progression tab. Users can now filter Full Student View by specific semester (Fall-1, Spring-2, etc.) or semester type (Fall, Spring, Summer) to see semester-grouped data. When filtering is active, displays: semester name as header (📅 Fall-1), greyed-out REQUISITES subheading with course prerequisites, greyed-out SUMMARY subheading with aggregate stats (c:X | r:Y | s:Z...), followed by the fully sortable student data table.
- **2025-11-06**: Formatted Total Credits Completed and Remaining Credits to display as whole numbers without decimal places for cleaner presentation.
...
- **Student Views**: Provides multiple views - "Student Eligibility View" (courses a student can take) and "Full Student View" (complete progress tracking with optional semester filtering). Full Student View includes semester filtering to show courses by specific semester (Fall-1, Spring-2, etc.) or semester type (Fall, Spring, Summer), with greyed-out REQUISITES and SUMMARY subheadings when filtering is active.
- **Course Offering Planner**: Smart decision support tool that recommends which courses to offer each semester based on multiple factors: graduating students (adjustable remaining credits threshold), bottleneck analysis (identifies courses that unlock many downstream prerequisites), currently eligible student count, and cascading eligibility (simulates how many additional students become eligible for other courses). Uses weighted priority scoring (3x graduating, 2x bottleneck, 1.5x cascading, 1x base eligible) to rank recommendations. Includes Top 10 visual cards with color-coded priority indicators, interactive course selection, and impact summaries showing total eligible and graduating students served.
