...
def send_student_email(session: Session, *, major_code: str, student_id: str, template_key: str = 'default') -> dict:
    ...
    subject = template.subject_template.format(major=major_code, student_name=payload.student_name, semester='Current', year='')
    body = template.body_template.format(student_name=payload.student_name, major=major_code, semester='Current', year='')
